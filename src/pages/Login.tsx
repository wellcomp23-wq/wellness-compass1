...
export default function LoginPage() {
  ...
  const handlePatientLogin = async () => {
    ...
    try {
      ...
      if (data.user && isMounted.current) {
        ...
        if (userRole !== "PATIENT") {
          throw new Error("هذا الحساب ليس لمريض. يرجى استخدام بيانات اعتماد صحيحة.");
        }

        const redirectPath = getRoleBasedRedirect(userRole as UserRole);
        
        // Show welcome toast only once on successful login
        if (isMounted.current) {
          toast({
            title: "مرحباً بك في بوصلة العافية",
  ...
  const handleProviderLogin = async () => {
    ...
    try {
      console.log("Attempting provider login for:", trimmedUsername);
      
      // الذكاء هنا: سنحاول تسجيل الدخول مباشرة أولاً إذا كان المدخل بريداً إلكترونياً
      // هذا يتجاوز مشاكل الـ RLS في البحث الأولي
      let loginEmail = trimmedUsername;
      const isEmail = trimmedUsername.includes('@');

      if (!isEmail) {
        // إذا لم يكن بريداً، نبحث عنه في جدول المستخدمين للحصول على البريد المرتبط
        ...
        if (userError || !userData) {
          ...
          throw new Error("اسم المستخدم غير صحيح أو غير مسجل كمقدم خدمة");
        }
        
        if (userData.role === "PATIENT") {
          throw new Error("هذا الحساب خاص بمريض.");
        }
        
        if (userData.account_status !== "ACTIVE") {
          throw new Error("حسابك قيد المراجعة حالياً.");
        }
        
        loginEmail = userData.email;
      }

      // تنفيذ تسجيل الدخول الرسمي
      console.log("Executing Auth sign-in for:", loginEmail);
      const { data, error } = await supabase.auth.signInWithPassword({
        email: loginEmail,
        password: trimmedPassword,
      });
  ...
  const handleGoogleLogin = async () => {
    if (loginType !== "patient") {
      toast({ title: "تنبيه", description: "خيار Google متاح فقط للمرضى", variant: "destructive" });
      return;
    }
        ...
        <Card className="border-none shadow-xl bg-white rounded-3xl overflow-hidden">
          ...
          <CardContent className="pt-6 space-y-6">
            ...
            <div className="flex gap-2 bg-slate-100 p-1 rounded-lg">
              ...
              >
                <Stethoscope className="w-4 h-4 inline-block ml-2" />
                مقدم خدمة
              </button>
            </div>
              ...
              {loginType === "patient" ? (
                ...
                <>
                  <div className="space-y-2">
                    <Label htmlFor="email-username">البريد الإلكتروني</Label>
                ...
                <>
                  <div className="space-y-2">
                    <Label htmlFor="email-username">اسم المستخدم</Label>
                    <div className="relative">
                      <div className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400">
