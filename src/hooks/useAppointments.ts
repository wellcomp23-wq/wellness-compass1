...
export function useAppointments(providedPatientId?: string) {
  const [appointments, setAppointments] = useState<Appointment[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [patientId, setPatientId] = useState<string | undefined>(providedPatientId)
  ...
  const updateAppointment = async (id: string, updates: Partial<Appointment>) => {
    try {
      ...
      // Fetch fresh data to ensure doctor info is included
      await fetchAppointments()
      
      return data
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'فشل في تحديث الموعد'
