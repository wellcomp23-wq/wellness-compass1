  ...
  const handleUpdateAppointment = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error.message || "حدث خطأ أثناء محاولة تحديث الموعد",
        variant: "destructive"
      })
    } finally {
      setIsLoading(false)
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md w-[95%] rounded-[2.5rem] p-8 overflow-y-auto max-h-[90vh]" dir="rtl">
        <DialogHeader className="text-right">
          <DialogTitle className="text-2xl font-black">تعديل الموعد</DialogTitle>
          <DialogDescription className="text-muted-foreground mt-2">
            تعديل تفاصيل موعدك مع {appointment?.doctor ? `د. ${appointment.doctor.first_name} ${appointment.doctor.last_name}` : 'الطبيب'}
          </DialogDescription>
        </DialogHeader>
